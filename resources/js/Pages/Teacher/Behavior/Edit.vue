...
const form = useForm({
    title: props.behavior.title || '',
    date: props.behavior.date || '',
    class_id: props.filters?.class_id || '',
    section_id: props.filters?.section_id || '',
    student_id: props.behavior.student_id || '',
    note: props.behavior.note || '',
    attachment: null,
});

const fetchSections = () => {
    if (form.class_id) {
        router.get(route('teacher.behaviors.edit', props.behavior.id), {
...
const submit = () => {
    ...
    // Workaround for sending files via PUT/PATCH in Inertia
    // We send a POST request and add _method: 'PUT'
    const payload = new FormData();
    payload.append('_method', 'PUT');
    payload.append('title', submissionData.title);
    payload.append('date', submissionData.date);
    payload.append('class_id', submissionData.class_id);
    payload.append('section_id', submissionData.section_id);
    payload.append('student_id', submissionData.student_id);
    if (submissionData.note) payload.append('note', submissionData.note);
    if (submissionData.attachment) payload.append('attachment', submissionData.attachment);

    router.post(route('teacher.behaviors.update', props.behavior.id), payload, {
        preserveScroll: true,
        onSuccess: () => {
...
<template>
    <TeacherLayout title="Behavior Edit">
        <div class="mb-4 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Behavior</h1>
                <p class="text-sm text-gray-500">Home - Behaviors - Edit</p>
            </div>
            <Link :href="route('teacher.behaviors.index')">
        ...
        <Card class="shadow-sm border border-slate-100 rounded-xl bg-white max-w-5xl">
            <template #content>
                <form @submit.prevent="submit" class="flex flex-col gap-6 pt-2">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Attachment (Leave blank to keep current file)</label>
                    ...
                    <div class="mt-4">
                        <Button type="submit" label="Update" :loading="form.processing" class="bg-[#0ea5e9] border-none text-white hover:bg-sky-600 px-8" />
                    </div>
                </form>
            </template>
        </Card>
