...
const fetchBehaviors = debounce(() => {
    router.get(route('teacher.behaviors.index'), {
...
const deleteBehavior = (id) => {
    confirm.require({
        ...
        accept: () => {
            router.delete(route('teacher.behaviors.destroy', id), {
                preserveScroll: true,
            });
        },
    });
...
<template>
    <TeacherLayout title="Behavior">
        ...
        <div class="mb-4 flex items-center justify-between">
            ...
            <Link :href="route('teacher.behaviors.create')">
                            ...
                            <template v-if="behaviors.data.length > 0">
                                <tr v-for="(behavior, index) in behaviors.data" :key="behavior.id" class="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 text-blue-500">
                                        <a v-if="behavior.attachment" :href="route('teacher.behaviors.download', behavior.id)" target="_blank" class="hover:underline">Download</a>
                                        <span v-else class="text-gray-400">-</span>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <Link :href="route('teacher.behaviors.edit', behavior.id)">
