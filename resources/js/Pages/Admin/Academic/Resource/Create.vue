...
const fetchSections = () => {
    if (form.class_id) {
        router.get(route('admin.academic.resources.create'), {
...
const submit = () => {
    ...
    })).post(route('admin.academic.resources.store'), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <AdminLayout title="Add New Resource">
        <div class="mb-4 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Resource</h1>
                <p class="text-sm text-gray-500">Home - Resources - Create</p>
            </div>
            <Link :href="route('admin.academic.resources.index')">
                <Button label="Resource List" class="bg-[#0ea5e9] border-none text-white hover:bg-sky-600" />
            </Link>
        </div>

        <Card class="shadow-sm border border-slate-100 rounded-xl bg-white max-w-4xl">
            <template #title>
                <h2 class="text-xl font-bold text-slate-800 pb-2">Add New Resource</h2>
            </template>
            <template #content>
                <form @submit.prevent="submit" class="flex flex-col gap-6">
                    ...
                    <div class="mt-4">
                        <Button type="submit" label="Add Resource" :loading="form.processing" class="bg-[#0ea5e9] border-none text-white hover:bg-sky-600" />
                    </div>
                </form>
            </template>
        </Card>
