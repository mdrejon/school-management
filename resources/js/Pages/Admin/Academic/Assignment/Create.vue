...
const fetchSections = () => {
    if (form.class_id) {
        router.get(route('admin.academic.assignments.create'), {
...
const submit = () => {
    ...
    })).post(route('admin.academic.assignments.store'), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <AdminLayout title="Add New Assignment">
        <div class="mb-4">
            <h1 class="text-2xl font-bold text-gray-800">Dashboard</h1>
            <p class="text-sm text-gray-500">Home - Admin - Create-assignment</p>
        </div>
            ...
            <template #content>
                <form @submit.prevent="submit" class="flex flex-col gap-6">
                    ...
                    <div>
                        ...
                        <Dropdown v-model="form.teacher_id" :options="teachers" optionLabel="name" optionValue="id" placeholder="Select Teacher" class="w-full" :class="{ 'p-invalid': form.errors.teacher_id }" filter />
                        <small v-if="form.errors.teacher_id" class="p-error">{{ form.errors.teacher_id }}</small>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Subject <span class="text-red-500">*</span></label>
                        <Dropdown v-model="form.subject_id" :options="subjects" optionLabel="name" optionValue="id" placeholder="Select Subject" class="w-full" :class="{ 'p-invalid': form.errors.subject_id }" filter />
