...
const toggleActive = (facility) => {
    router.patch(route('admin.cms.facilities.toggle', facility.id), {}, { preserveScroll: true });
};
const moveUp = (facility) => {
    router.patch(route('admin.cms.facilities.moveUp', facility.id), {}, { preserveScroll: true });
};
const moveDown = (facility) => {
    router.patch(route('admin.cms.facilities.moveDown', facility.id), {}, { preserveScroll: true });
};
const confirmDelete = (facility) => {
    confirm.require({
        message: 'Remove this facility? This can\'t be undone.',
        header: 'Remove facility',
        icon: 'pi pi-exclamation-triangle',
        acceptProps: { label: 'Remove', severity: 'danger' },
        rejectProps: { label: 'Cancel', severity: 'secondary', outlined: true },
        accept: () => router.delete(route('admin.cms.facilities.destroy', facility.id), { preserveScroll: true }),
    });
};

const defaultText = (facility, field) => {
    const defaultCode = defaultLangCode.value;
    return facility[field]?.[defaultCode] ?? Object.values(facility[field] ?? {})[0] ?? '—';
};
...
const submitSettings = () => {
    settingsForm.transform((data) => ({ ...data, _method: 'put' })).post(route('admin.cms.facilities.settings.update'), {
        forceFormData: true,
        preserveScroll: true,
    });
};
...
<template>
    <AdminLayout title="Facilities">
        <p class="text-sm text-slate-500 mb-6">
            Manage the facilities shown on the public site, and the facilities list page's own heading, breadcrumb, and SEO config.
        </p>
                    ...
                    <Card class="shadow-sm">
                        <template #title>
                            <div class="flex items-center justify-between gap-4">
                                <div>
                                    <div class="text-lg font-semibold">Facilities</div>
                                    <p class="text-sm font-normal text-slate-500 mt-1">
                                        Shown on the public facilities list/detail pages.
                                    </p>
                                </div>
                                <Link :href="route('admin.cms.facilities.create')">
                                    <Button label="Add Facility" icon="pi pi-plus" as="span" />
                                </Link>
                            </div>
                        </template>
                        <template #content>
                            <div class="overflow-x-auto">
                                <DataTable :value="facilities" data-key="id" size="small">
                                    <Column header="Order" style="width: 90px">
                                        <template #body="{ data, index }">
                                            <div class="flex items-center gap-1">
                                                ...
                                                <Button icon="pi pi-angle-down" text rounded size="small" :disabled="index === facilities.length - 1" @click="moveDown(data)" />
                                            </div>
                                        </template>
                                    </Column>
                                    ...
                                    <Column header="Facility">
                                        <template #body="{ data }">
                                            ...
                                            <div class="text-xs text-slate-500 mt-1">{{ defaultText(data, 'short_description') }}</div>
                                        </template>
                                    </Column>
                                    ...
                                    <Column header="" style="width: 100px">
                                        <template #body="{ data }">
                                            <div class="flex items-center gap-1 justify-end">
                                                <Link :href="route('admin.cms.facilities.edit', data.id)">
                            ...
                            <div class="flex flex-col gap-5">
                                <section class="rounded-xl border border-slate-200 p-5">
                                    ...
                                    <div class="flex flex-col gap-4 max-w-lg">
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_tagline[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Our Facilities" />
                                        ...
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_title[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Let's Check Our Facilities" />
                                        ...
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_highlight[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Facilities" />
                                ...
                                <section class="rounded-xl border border-slate-200 p-5">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 mb-1.5">Title</label>
                                            <InputText v-model="settingsForm.breadcrumb_title[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Our Facilities" />
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 mb-1.5">Background image</label>
                                            <ImageDropzone
                                                :preview-url="breadcrumbImagePreview"
                                                hint="Shown behind the page title on the facilities list page"
