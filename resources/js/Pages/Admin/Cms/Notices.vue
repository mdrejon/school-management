...
const activeMainTab = ref('notices');

const toggleActive = (notice) => {
    router.patch(route('admin.cms.notices.toggle', notice.id), {}, { preserveScroll: true });
};
const confirmDelete = (notice) => {
    confirm.require({
        message: 'Remove this notice? This can\'t be undone.',
        header: 'Remove notice',
        icon: 'pi pi-exclamation-triangle',
        acceptProps: { label: 'Remove', severity: 'danger' },
        rejectProps: { label: 'Cancel', severity: 'secondary', outlined: true },
        accept: () => router.delete(route('admin.cms.notices.destroy', notice.id), { preserveScroll: true }),
    });
};

const defaultText = (notice, field) => {
    const defaultCode = defaultLangCode.value;
    return notice[field]?.[defaultCode] ?? Object.values(notice[field] ?? {})[0] ?? '—';
};
...
const settingsForm = useForm({
    marquee_label: { ...emptyTranslatable(), ...props.pageSettings.marquee_label },
...
const submitSettings = () => {
    settingsForm.transform((data) => ({ ...data, _method: 'put' })).post(route('admin.cms.notices.settings.update'), {
        forceFormData: true,
        preserveScroll: true,
    });
};
...
<template>
    <AdminLayout title="Notices">
        <p class="text-sm text-slate-500 mb-6">
            Manage the notices shown in the site-wide header ticker and the public notices list/detail pages.
        </p>

        <Tabs v-model:value="activeMainTab">
            <TabList>
                <Tab value="notices">Notices</Tab>
                    ...
                    <Card class="shadow-sm">
                        <template #title>
                            <div class="flex items-center justify-between gap-4">
                                <div>
                                    <div class="text-lg font-semibold">Notices</div>
                                    <p class="text-sm font-normal text-slate-500 mt-1">
                                        Newest publish date shows first in the header ticker and the notices list.
                                    </p>
                                </div>
                                <Link :href="route('admin.cms.notices.create')">
                                    <Button label="Add Notice" icon="pi pi-plus" as="span" />
                                </Link>
                            </div>
                        </template>
                        <template #content>
                            <div class="overflow-x-auto">
                                <DataTable :value="notices" data-key="id" size="small">
                                    ...
                                    <Column header="Publish Date" style="width: 140px">
                                        <template #body="{ data }">
                                            <span class="text-sm text-slate-700">{{ data.published_at ?? '—' }}</span>
                                        </template>
                                    </Column>

                                    <Column header="PDF" style="width: 90px">
                                        <template #body="{ data }">
                                            <a v-if="data.pdf_url" :href="data.pdf_url" target="_blank" class="text-indigo-600 text-sm hover:underline">
                                                <i class="pi pi-file-pdf"></i> View
                                            </a>
                                    ...
                                    <Column header="" style="width: 100px">
                                        <template #body="{ data }">
                                            <div class="flex items-center gap-1 justify-end">
                                                <Link :href="route('admin.cms.notices.edit', data.id)">
                        ...
                        <template #content>
                            ...
                            <div class="flex flex-col gap-5">
                                <section class="rounded-xl border border-slate-200 p-5">
                                    <h3 class="text-sm font-semibold text-slate-800 mb-3">Header ticker</h3>
                                    <div class="max-w-sm">
                                        <label class="block text-xs font-medium text-slate-600 mb-1.5">Label</label>
                                        <InputText v-model="settingsForm.marquee_label[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Notice" />
                                        <p class="text-xs text-slate-400 mt-1">Shown to the left of the scrolling notice ticker on every page.</p>
                                    </div>
                                </section>
                                ...
                                <section class="rounded-xl border border-slate-200 p-5">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.breadcrumb_title[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Notice" />
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 mb-1.5">Background image</label>
                                            <ImageDropzone
                                                :preview-url="breadcrumbImagePreview"
                                                hint="Shown behind the page title on the notices list page"
