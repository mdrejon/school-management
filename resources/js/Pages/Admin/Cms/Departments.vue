...
const activeMainTab = ref('departments');

const toggleActive = (department) => {
    router.patch(route('admin.cms.departments.toggle', department.id), {}, { preserveScroll: true });
};
const moveUp = (department) => {
    router.patch(route('admin.cms.departments.moveUp', department.id), {}, { preserveScroll: true });
};
const moveDown = (department) => {
    router.patch(route('admin.cms.departments.moveDown', department.id), {}, { preserveScroll: true });
};
const confirmDelete = (department) => {
    confirm.require({
        message: 'Remove this department? This can\'t be undone.',
        header: 'Remove department',
        icon: 'pi pi-exclamation-triangle',
        acceptProps: { label: 'Remove', severity: 'danger' },
        rejectProps: { label: 'Cancel', severity: 'secondary', outlined: true },
        accept: () => router.delete(route('admin.cms.departments.destroy', department.id), { preserveScroll: true }),
    });
};

const defaultText = (department, field) => {
    const defaultCode = defaultLangCode.value;
    return department[field]?.[defaultCode] ?? Object.values(department[field] ?? {})[0] ?? '—';
};
...
const submitSettings = () => {
    settingsForm.transform((data) => ({ ...data, _method: 'put' })).post(route('admin.cms.departments.settings.update'), {
        forceFormData: true,
        preserveScroll: true,
    });
};
...
<template>
    <AdminLayout title="Departments">
        <p class="text-sm text-slate-500 mb-6">
            Manage the academic departments shown on the public site, and the departments list page's own heading, breadcrumb, and SEO config.
        </p>
                    ...
                    <Card class="shadow-sm">
                        <template #title>
                            <div class="flex items-center justify-between gap-4">
                                <div>
                                    <div class="text-lg font-semibold">Departments</div>
                                    <p class="text-sm font-normal text-slate-500 mt-1">
                                        Shown on the homepage teaser and the public departments list/detail pages.
                                    </p>
                                </div>
                                <Link :href="route('admin.cms.departments.create')">
                                    <Button label="Add Department" icon="pi pi-plus" as="span" />
                                </Link>
                            </div>
                        </template>
                        <template #content>
                            <div class="overflow-x-auto">
                                <DataTable :value="departments" data-key="id" size="small">
                                    <Column header="Order" style="width: 90px">
                                        <template #body="{ data, index }">
                                            <div class="flex items-center gap-1">
                                                ...
                                                <Button icon="pi pi-angle-down" text rounded size="small" :disabled="index === departments.length - 1" @click="moveDown(data)" />
                                            </div>
                                        </template>
                                    </Column>
                                    ...
                                    <Column header="" style="width: 100px">
                                        <template #body="{ data }">
                                            <div class="flex items-center gap-1 justify-end">
                                                <Link :href="route('admin.cms.departments.edit', data.id)">
                            ...
                            <div class="flex flex-col gap-5">
                                <section class="rounded-xl border border-slate-200 p-5">
                                    ...
                                    <div class="flex flex-col gap-4 max-w-lg">
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_tagline[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Department" />
                                        ...
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_title[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Browse Our Department" />
                                        ...
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_highlight[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Department" />
                                ...
                                <section class="rounded-xl border border-slate-200 p-5">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 mb-1.5">Title</label>
                                            <InputText v-model="settingsForm.breadcrumb_title[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Academics Department" />
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 mb-1.5">Background image</label>
                                            <ImageDropzone
                                                :preview-url="breadcrumbImagePreview"
                                                hint="Shown behind the page title on the departments list page"
