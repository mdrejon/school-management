...
const activeMainTab = ref('courses');

const toggleActive = (course) => {
    router.patch(route('admin.cms.courses.toggle', course.id), {}, { preserveScroll: true });
};
const moveUp = (course) => {
    router.patch(route('admin.cms.courses.moveUp', course.id), {}, { preserveScroll: true });
};
const moveDown = (course) => {
    router.patch(route('admin.cms.courses.moveDown', course.id), {}, { preserveScroll: true });
};
const confirmDelete = (course) => {
    confirm.require({
        message: 'Remove this course? This can\'t be undone.',
        header: 'Remove course',
        icon: 'pi pi-exclamation-triangle',
        acceptProps: { label: 'Remove', severity: 'danger' },
        rejectProps: { label: 'Cancel', severity: 'secondary', outlined: true },
        accept: () => router.delete(route('admin.cms.courses.destroy', course.id), { preserveScroll: true }),
    });
};

const defaultText = (course, field) => {
    const defaultCode = defaultLangCode.value;
    return course[field]?.[defaultCode] ?? Object.values(course[field] ?? {})[0] ?? '—';
};
...
const submitSettings = () => {
    settingsForm.transform((data) => ({ ...data, _method: 'put' })).post(route('admin.cms.courses.settings.update'), {
        forceFormData: true,
        preserveScroll: true,
    });
};
...
<template>
    <AdminLayout title="Courses">
        <p class="text-sm text-slate-500 mb-6">
            Manage the courses shown on the public site, and the courses list page's own heading, breadcrumb, and SEO config.
        </p>
                            ...
                            <div class="flex items-center justify-between gap-4">
                                <div>
                                    ...
                                    <p class="text-sm font-normal text-slate-500 mt-1">
                                        Shown on the homepage teaser and the public courses list/detail pages.
                                    </p>
                                </div>
                                <Link :href="route('admin.cms.courses.create')">
                                    <Button label="Add Course" icon="pi pi-plus" as="span" />
                                </Link>
                            </div>
                        </template>
                        <template #content>
                            <div class="overflow-x-auto">
                                <DataTable :value="courses" data-key="id" size="small">
                                    <Column header="Order" style="width: 90px">
                                        <template #body="{ data, index }">
                                            <div class="flex items-center gap-1">
                                                ...
                                                <Button icon="pi pi-angle-down" text rounded size="small" :disabled="index === courses.length - 1" @click="moveDown(data)" />
                                            </div>
                                        </template>
                                    </Column>

                                    <Column header="Thumbnail" style="width: 110px">
                                        <template #body="{ data }">
                                            <img :src="data.thumbnail_url" class="w-20 h-14 object-cover bg-slate-100 rounded-md border border-slate-200" alt="" />
                                        </template>
                                    </Column>
                                    ...
                                    <Column header="Price / Duration">
                                        <template #body="{ data }">
                                            <div class="text-sm text-slate-700">{{ data.price || '—' }}</div>
                                            <div class="text-xs text-slate-500">{{ defaultText(data, 'duration') }}</div>
                                        </template>
                                    </Column>
                                    ...
                                    <Column header="" style="width: 100px">
                                        <template #body="{ data }">
                                            <div class="flex items-center gap-1 justify-end">
                                                <Link :href="route('admin.cms.courses.edit', data.id)">
                            ...
                            <div class="flex flex-col gap-5">
                                <section class="rounded-xl border border-slate-200 p-5">
                                    ...
                                    <div class="flex flex-col gap-4 max-w-lg">
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_tagline[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Our Courses" />
                                        ...
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_title[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Let's Check Our Courses" />
                                        ...
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_highlight[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Courses" />
                                ...
                                <section class="rounded-xl border border-slate-200 p-5">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 mb-1.5">Title</label>
                                            <InputText v-model="settingsForm.breadcrumb_title[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Our Courses" />
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 mb-1.5">Background image</label>
                                            <ImageDropzone
                                                :preview-url="breadcrumbImagePreview"
                                                hint="Shown behind the page title on the courses list page"
