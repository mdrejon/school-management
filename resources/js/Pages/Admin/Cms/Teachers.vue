...
const activeMainTab = ref('teachers');

const toggleActive = (teacher) => {
    router.patch(route('admin.cms.teachers.toggle', teacher.id), {}, { preserveScroll: true });
};
const moveUp = (teacher) => {
    router.patch(route('admin.cms.teachers.moveUp', teacher.id), {}, { preserveScroll: true });
};
const moveDown = (teacher) => {
    router.patch(route('admin.cms.teachers.moveDown', teacher.id), {}, { preserveScroll: true });
};
const confirmDelete = (teacher) => {
    confirm.require({
        message: 'Remove this teacher? This can\'t be undone.',
        header: 'Remove teacher',
        icon: 'pi pi-exclamation-triangle',
        acceptProps: { label: 'Remove', severity: 'danger' },
        rejectProps: { label: 'Cancel', severity: 'secondary', outlined: true },
        accept: () => router.delete(route('admin.cms.teachers.destroy', teacher.id), { preserveScroll: true }),
    });
};

const defaultText = (teacher, field) => {
    const defaultCode = defaultLangCode.value;
    return teacher[field]?.[defaultCode] ?? Object.values(teacher[field] ?? {})[0] ?? '—';
};
...
const submitSettings = () => {
    settingsForm.transform((data) => ({ ...data, _method: 'put' })).post(route('admin.cms.teachers.settings.update'), {
        forceFormData: true,
        preserveScroll: true,
    });
};
...
<template>
    <AdminLayout title="Teachers">
        <p class="text-sm text-slate-500 mb-6">
            Manage the teachers shown on the public site, and the teachers list page's own heading, breadcrumb, and SEO config.
        </p>
                            ...
                            <div class="flex items-center justify-between gap-4">
                                <div>
                                    ...
                                    <p class="text-sm font-normal text-slate-500 mt-1">
                                        Shown on the homepage teaser and the public teachers list/detail pages.
                                    </p>
                                </div>
                                <Link :href="route('admin.cms.teachers.create')">
                                    <Button label="Add Teacher" icon="pi pi-plus" as="span" />
                                </Link>
                            </div>
                        </template>
                        <template #content>
                            <div class="overflow-x-auto">
                                <DataTable :value="teachers" data-key="id" size="small">
                                    <Column header="Order" style="width: 90px">
                                        <template #body="{ data, index }">
                                            <div class="flex items-center gap-1">
                                                ...
                                                <Button icon="pi pi-angle-down" text rounded size="small" :disabled="index === teachers.length - 1" @click="moveDown(data)" />
                                            </div>
                                        </template>
                                    </Column>
                                    ...
                                    <Column header="Photo" style="width: 90px">
                                        <template #body="{ data }">
                                            <img :src="data.photo_url" class="w-14 h-14 object-cover bg-slate-100 rounded-full border border-slate-200" alt="" />
                                        </template>
                                    </Column>
                                    ...
                                    <Column header="Contact">
                                        <template #body="{ data }">
                                            <div class="text-sm text-slate-700">{{ data.email || '—' }}</div>
                                    ...
                                    <Column header="" style="width: 100px">
                                        <template #body="{ data }">
                                            <div class="flex items-center gap-1 justify-end">
                                                <Link :href="route('admin.cms.teachers.edit', data.id)">
                            ...
                            <div class="flex flex-col gap-5">
                                <section class="rounded-xl border border-slate-200 p-5">
                                    ...
                                    <div class="flex flex-col gap-4 max-w-lg">
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_tagline[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Our Teachers" />
                                        ...
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_title[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Meet With Our Teachers" />
                                        ...
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_highlight[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Teachers" />
                                ...
                                <section class="rounded-xl border border-slate-200 p-5">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 mb-1.5">Title</label>
                                            <InputText v-model="settingsForm.breadcrumb_title[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Teachers" />
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 mb-1.5">Background image</label>
                                            <ImageDropzone
                                                :preview-url="breadcrumbImagePreview"
                                                hint="Shown behind the page title on the teachers list page"
