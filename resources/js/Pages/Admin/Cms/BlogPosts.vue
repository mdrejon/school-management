...
const toggleActive = (post) => {
    router.patch(route('admin.cms.blog.toggle', post.id), {}, { preserveScroll: true });
};
const moveUp = (post) => {
    router.patch(route('admin.cms.blog.moveUp', post.id), {}, { preserveScroll: true });
};
const moveDown = (post) => {
    router.patch(route('admin.cms.blog.moveDown', post.id), {}, { preserveScroll: true });
};
const confirmDelete = (post) => {
    confirm.require({
        message: 'Remove this post? This can\'t be undone.',
        header: 'Remove post',
        icon: 'pi pi-exclamation-triangle',
        acceptProps: { label: 'Remove', severity: 'danger' },
        rejectProps: { label: 'Cancel', severity: 'secondary', outlined: true },
        accept: () => router.delete(route('admin.cms.blog.destroy', post.id), { preserveScroll: true }),
    });
};

const defaultText = (post, field) => {
    const defaultCode = defaultLangCode.value;
    return post[field]?.[defaultCode] ?? Object.values(post[field] ?? {})[0] ?? '—';
};
...
const submitSettings = () => {
    settingsForm.transform((data) => ({ ...data, _method: 'put' })).post(route('admin.cms.blog.settings.update'), {
        forceFormData: true,
        preserveScroll: true,
    });
};
...
<template>
    <AdminLayout title="Blog">
        <p class="text-sm text-slate-500 mb-6">
            Manage the blog posts shown on the homepage teaser and the public blog pages, and the blog list page's own heading, breadcrumb, and SEO config.
        </p>
                            ...
                            <div class="flex items-center justify-between gap-4">
                                <div>
                                    ...
                                    <p class="text-sm font-normal text-slate-500 mt-1">
                                        Shown on the homepage teaser and the public blog list/detail pages.
                                    </p>
                                </div>
                                <Link :href="route('admin.cms.blog.create')">
                                    <Button label="Add Post" icon="pi pi-plus" as="span" />
                                </Link>
                            </div>
                        </template>
                        <template #content>
                            <div class="overflow-x-auto">
                                <DataTable :value="posts" data-key="id" size="small">
                                    <Column header="Order" style="width: 90px">
                                        <template #body="{ data, index }">
                                            <div class="flex items-center gap-1">
                                                ...
                                                <Button icon="pi pi-angle-down" text rounded size="small" :disabled="index === posts.length - 1" @click="moveDown(data)" />
                                            </div>
                                        </template>
                                    </Column>
                                    ...
                                    <Column header="Post">
                                        <template #body="{ data }">
                                            ...
                                            <div class="text-xs text-slate-500 mt-1">{{ defaultText(data, 'author_name') }}</div>
                                        </template>
                                    </Column>

                                    <Column header="Published" style="width: 130px">
                                        <template #body="{ data }">
                                            <span class="text-sm text-slate-700">{{ data.published_at ?? '—' }}</span>
                                        </template>
                                    </Column>
                                    ...
                                    <Column header="" style="width: 100px">
                                        <template #body="{ data }">
                                            <div class="flex items-center gap-1 justify-end">
                                                <Link :href="route('admin.cms.blog.edit', data.id)">
                            ...
                            <div class="flex flex-col gap-5">
                                <section class="rounded-xl border border-slate-200 p-5">
                                    ...
                                    <div class="flex flex-col gap-4 max-w-lg">
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_tagline[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Our Blog" />
                                        ...
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_title[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Latest News & Blog" />
                                        ...
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_highlight[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Blog" />
                                ...
                                <section class="rounded-xl border border-slate-200 p-5">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 mb-1.5">Title</label>
                                            <InputText v-model="settingsForm.breadcrumb_title[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Our Blog" />
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 mb-1.5">Background image</label>
                                            <ImageDropzone
                                                :preview-url="breadcrumbImagePreview"
                                                hint="Shown behind the page title on the blog list page"
