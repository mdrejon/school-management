...
const activeMainTab = ref('portfolios');

const toggleActive = (portfolio) => {
    router.patch(route('admin.cms.portfolios.toggle', portfolio.id), {}, { preserveScroll: true });
};
const moveUp = (portfolio) => {
    router.patch(route('admin.cms.portfolios.moveUp', portfolio.id), {}, { preserveScroll: true });
};
const moveDown = (portfolio) => {
    router.patch(route('admin.cms.portfolios.moveDown', portfolio.id), {}, { preserveScroll: true });
};
const confirmDelete = (portfolio) => {
    confirm.require({
        message: 'Remove this portfolio item? This can\'t be undone.',
        header: 'Remove portfolio item',
        icon: 'pi pi-exclamation-triangle',
        acceptProps: { label: 'Remove', severity: 'danger' },
        rejectProps: { label: 'Cancel', severity: 'secondary', outlined: true },
        accept: () => router.delete(route('admin.cms.portfolios.destroy', portfolio.id), { preserveScroll: true }),
    });
};

const defaultText = (portfolio, field) => {
    const defaultCode = defaultLangCode.value;
    return portfolio[field]?.[defaultCode] ?? Object.values(portfolio[field] ?? {})[0] ?? '—';
};
...
const submitSettings = () => {
    settingsForm.transform((data) => ({ ...data, _method: 'put' })).post(route('admin.cms.portfolios.settings.update'), {
        forceFormData: true,
        preserveScroll: true,
    });
};
...
<template>
    <AdminLayout title="Portfolio">
        <p class="text-sm text-slate-500 mb-6">
            Manage the portfolio items shown on the public site, and the portfolio list page's own heading, breadcrumb, and SEO config.
        </p>

        <Tabs v-model:value="activeMainTab">
            <TabList>
                <Tab value="portfolios">Portfolio</Tab>
                    ...
                    <Card class="shadow-sm">
                        <template #title>
                            <div class="flex items-center justify-between gap-4">
                                <div>
                                    <div class="text-lg font-semibold">Portfolio</div>
                                    <p class="text-sm font-normal text-slate-500 mt-1">
                                        Shown on the public portfolio list/detail pages.
                                    </p>
                                </div>
                                <Link :href="route('admin.cms.portfolios.create')">
                                    <Button label="Add Portfolio Item" icon="pi pi-plus" as="span" />
                                </Link>
                            </div>
                        </template>
                        <template #content>
                            <div class="overflow-x-auto">
                                <DataTable :value="portfolios" data-key="id" size="small">
                                    <Column header="Order" style="width: 90px">
                                        <template #body="{ data, index }">
                                            <div class="flex items-center gap-1">
                                                ...
                                                <Button icon="pi pi-angle-down" text rounded size="small" :disabled="index === portfolios.length - 1" @click="moveDown(data)" />
                                            </div>
                                        </template>
                                    </Column>
                                    ...
                                    <Column header="Portfolio">
                                        <template #body="{ data }">
                                            <div class="font-medium text-slate-800">{{ defaultText(data, 'title') }}</div>
                                            <Tag :value="defaultText(data, 'category')" severity="secondary" class="mt-1" />
                                        </template>
                                    </Column>
                                    ...
                                    <Column header="" style="width: 100px">
                                        <template #body="{ data }">
                                            <div class="flex items-center gap-1 justify-end">
                                                <Link :href="route('admin.cms.portfolios.edit', data.id)">
                        ...
                        <template #content>
                            ...
                            <div class="flex flex-col gap-5">
                                <section class="rounded-xl border border-slate-200 p-5">
                                    <h3 class="text-sm font-semibold text-slate-800 mb-3">Page heading</h3>
                                    <div class="flex flex-col gap-4 max-w-lg">
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 mb-1.5">Tagline</label>
                                            <InputText v-model="settingsForm.section_tagline[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Portfolio" />
                                        ...
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_title[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Explore Our Portfolio" />
                                        ...
                                        <div>
                                            ...
                                            <InputText v-model="settingsForm.section_highlight[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Portfolio" />
                                ...
                                <section class="rounded-xl border border-slate-200 p-5">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 mb-1.5">Title</label>
                                            <InputText v-model="settingsForm.breadcrumb_title[activeLang]" :dir="currentLang?.direction" class="w-full" placeholder="e.g. Portfolio" />
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 mb-1.5">Background image</label>
                                            <ImageDropzone
                                                :preview-url="breadcrumbImagePreview"
                                                hint="Shown behind the page title on the portfolio list page"
