...
@section('content')
    ...
    <div class="wexnix_site-breadcrumb" @if($settings->academic_result_page_breadcrumb_image_url) style="background: url('{{ $settings->academic_result_page_breadcrumb_image_url }}')" @else style="background: url('{{ asset('frontend/assets/img/breadcrumb/01.jpg') }}')" @endif>
        <div class="container">
            <h2 class="wexnix_breadcrumb-title">{{ $settings->academic_result_page_breadcrumb_title ?? 'Academic Result' }}</h2>
            <ul class="wexnix_breadcrumb-menu">
                <li><a href="{{ route('home') }}">{{ __('Home') }}</a></li>
                <li class="active">{{ $settings->academic_result_page_breadcrumb_title ?? 'Academic Result' }}</li>
            </ul>
        </div>
    </div>
    <!-- breadcrumb end -->
                ...
                <form action="#">
                    <div class="row g-3 align-items-end">
                        <div class="col-md-6 col-lg-4">
                            <label class="wexnix_filter-label">Exam &amp; Session <span class="text-danger">*</span></label>
                            <select class="form-select">
                                <option>Select A Class, Exam &amp; Session</option>
                                <option>Class 6 - Annual Exam - 2026</option>
                                <option>Class 7 - Annual Exam - 2026</option>
                                <option>Class 8 - Annual Exam - 2026</option>
                            </select>
                        </div>
