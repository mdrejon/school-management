...
@section('content')
    <main class="wexnix_main">
        ...
        <div class="wexnix_site-breadcrumb" style="background: url({{ $pageSettings->breadcrumb_image_url ?? '/frontend/assets/img/breadcrumb/01.jpg' }})">
            <div class="container">
                <h2 class="wexnix_breadcrumb-title">{{ $pageSettings->{{ __('breadcrumb_title ?: \'Academics Department\' }}') }}</h2>
                <ul class="wexnix_breadcrumb-menu">
                    <li><a href="{{ route('home') }}">{{ __('Home') }}</a></li>
                    <li class="active">{{ $pageSettings->{{ __('breadcrumb_title ?: \'Academics Department\' }}') }}</li>
                </ul>
            </div>
        </div>
        <!-- breadcrumb end -->
        ...
        <div class="wexnix_department-area py-120">
            <div class="container">
                ...
                @if ($departments->{{ __('count())') }}
                    ...
                    <!-- pagination -->
                    {{ $departments->{{ __('links(\'frontend.pagination.wexnix\') }}') }}
                    <!-- pagination end -->
                @else
                    <p class="text-center">{{ __('No departments available right now — please check back soon.') }}</p>
                @endif
            </div>
        </div>
        <!-- department area end -->
