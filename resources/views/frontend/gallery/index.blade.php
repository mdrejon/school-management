...
@section('content')
    <main class="wexnix_main">
        ...
        <div class="wexnix_site-breadcrumb" style="background: url({{ $pageSettings->breadcrumb_image_url ?? '/frontend/assets/img/breadcrumb/01.jpg' }})">
            <div class="container">
                <h2 class="wexnix_breadcrumb-title">{{ $pageSettings->{{ __('breadcrumb_title ?: \'Gallery\' }}') }}</h2>
                <ul class="wexnix_breadcrumb-menu">
                    <li><a href="{{ route('home') }}">{{ __('Home') }}</a></li>
                    <li class="active">{{ $pageSettings->{{ __('breadcrumb_title ?: \'Gallery\' }}') }}</li>
                </ul>
            </div>
        </div>
        <!-- breadcrumb end -->
        ...
        <div class="wexnix_gallery-area py-120">
            <div class="container">
                ...
                @if ($images->{{ __('count())') }}
                    ...
                    <!-- pagination end -->
                @else
                    <p class="text-center">{{ __('No images available right now — please check back soon.') }}</p>
                @endif
            </div>
        </div>
        <!-- gallery-area end -->
