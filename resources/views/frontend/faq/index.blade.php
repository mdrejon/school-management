...
@section('content')
    <main class="wexnix_main">
        ...
        <div class="wexnix_site-breadcrumb" style="background: url({{ $pageSettings->breadcrumb_image_url ?? '/frontend/assets/img/breadcrumb/01.jpg' }})">
            <div class="container">
                <h2 class="wexnix_breadcrumb-title">{{ $pageSettings->breadcrumb_title ?: "Faq's" }}</h2>
                <ul class="wexnix_breadcrumb-menu">
                    <li><a href="{{ route('home') }}">Home</a></li>
                    <li class="active">{{ $pageSettings->breadcrumb_title ?: "Faq's" }}</li>
                </ul>
            </div>
        </div>
        <!-- breadcrumb end -->
                            ...
                            @if ($pageSettings->section_title)
                                ...
                                <div class="wexnix_site-heading mb-3">
                                    @if ($pageSettings->section_tagline)
                                        <span class="wexnix_site-title-tagline justify-content-start"><i class="fas fa-book-open-reader"></i> {{ $pageSettings->section_tagline }}</span>
                                    @endif
                                    <h2 class="wexnix_site-title my-3">{!! $titleHtml !!}</h2>
                                </div>
                            @endif
                            @if ($pageSettings->section_description)
                                <p class="mb-4">{{ $pageSettings->section_description }}</p>
                            @endif
                            @if ($pageSettings->cta_button_text && $pageSettings->cta_button_url)
                                <a href="{{ $pageSettings->cta_button_url }}" class="wexnix_theme-btn mt-2">{{ $pageSettings->cta_button_text }}</a>
                            @endif
                        </div>
                    </div>
                    <div class="col-lg-6">
                                ...
                                @foreach ($faqs as $faq)
                                    <div class="accordion-item">
                                        <h2 class="accordion-header" id="heading{{ $faq->id }}">
                                            ...
                                            >
                                                <span><i class="fas fa-question"></i></span> {{ $faq->question }}
                                            </button>
                                        </h2>
                                        <div
                                            id="collapse{{ $faq->id }}"
                        ...
                        @else
                            <p>No frequently asked questions available right now — please check back soon.</p>
                        @endif
                    </div>
                </div>
            </div>
