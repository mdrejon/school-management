                                ...
                                <div class="wexnix_widget wexnix_portfolio-sidebar-content">
                                    ...
                                    <ul>
                                        @if ($portfolio->category)
                                            <li>{{ __('Category') }} <span>{{ $portfolio->category }}</span></li>
                                        @endif
                                        @if ($portfolio->event_date)
                                            <li>{{ __('Date') }} <span>{{ $portfolio->event_date->format('d F, Y') }}</span></li>
                                        @endif
                                        @if ($portfolio->client_name)
                                            <li>{{ __('Client Name') }} <span>{{ $portfolio->client_name }}</span></li>
                                        @endif
                                        @if ($portfolio->cost)
                                            <li>{{ __('Total Cost') }} <span>{{ $portfolio->cost }}</span></li>
                                        @endif
                                        @if ($portfolio->manager_name)
                                            <li>{{ __('Manager') }} <span>{{ $portfolio->manager_name }}</span></li>
                                        @endif
                                        @if ($portfolio->location)
                                            <li>{{ __('Location') }} <span>{{ $portfolio->location }}</span></li>
                                        @endif
                                        @if ($portfolio->website_url)
                                            <li>{{ __('Website') }} <span>{{ $portfolio->website_url }}</span></li>
                                        @endif
                                        @if ($portfolio->rating)
                                            <li>
                                                {{ __('Rating') }}
                            ...
                            <div class="wexnix_portfolio-details">
                                @if ($portfolio->image_url)
                                    <div class="wexnix_portfolio-details-img mb-30">
                                        <img loading="lazy" src="{{ $portfolio->image_url }}" alt="thumb">
                                    </div>
                                @endif
                                <div class="wexnix_portfolio-details">
                                        ...
                                        <div class="row">
                                            @if ($portfolio->gallery_image_1_url)
                                                <div class="col-md-6 mb-20">
                                                    <img loading="lazy" src="{{ $portfolio->gallery_image_1_url }}" alt="">
                                                </div>
                                            @endif
                                            @if ($portfolio->gallery_image_2_url)
                                                <div class="col-md-6 mb-20">
                                                    <img loading="lazy" src="{{ $portfolio->gallery_image_2_url }}" alt="">
                                                </div>
                                            @endif
                                        </div>
                                    @endif
