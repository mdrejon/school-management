...
@section('content')
    <main class="wexnix_main">
        ...
        <div class="wexnix_site-breadcrumb" style="background: url({{ $pageSettings->breadcrumb_image_url ?? '/frontend/assets/img/breadcrumb/01.jpg' }})">
            <div class="container">
                <h2 class="wexnix_breadcrumb-title">{{ $pageSettings->breadcrumb_title ?: 'Portfolio' }}</h2>
                <ul class="wexnix_breadcrumb-menu">
                    <li><a href="{{ route('home') }}">Home</a></li>
                    <li class="active">{{ $pageSettings->breadcrumb_title ?: 'Portfolio' }}</li>
                </ul>
            </div>
        </div>
        <!-- breadcrumb end -->
        ...
        <div class="wexnix_portfolio-area py-120">
            <div class="container">
                ...
                @if ($portfolios->count())
                    ...
                    <!-- pagination -->
                    {{ $portfolios->links('frontend.pagination.wexnix') }}
                    <!-- pagination end -->
                @else
                    <p class="text-center">No portfolio items available right now — please check back soon.</p>
                @endif
            </div>
        </div>
        <!-- portfolio-area end -->
