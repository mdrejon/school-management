...
@if ($siteSettings->{{ __('offer_title)') }}
                ...
                <div class="row align-items-center">
                    <div class="col-lg-5 ms-lg-auto">
                        <div class="wexnix_cta-content">
                            <h1>{{ $siteSettings->{{ __('offer_title }}') }}</h1>
                            @if ($siteSettings->{{ __('offer_description)') }}
                                <p>{{ $siteSettings->{{ __('offer_description }}') }}</p>
                            @endif
                            @if ($siteSettings->{{ __('offer_button_text)') }}
                                <div class="wexnix_cta-btn">
                                    <a href="{{ $siteSettings->offer_button_url ?: '#' }}" class="wexnix_theme-btn">{{ $siteSettings->{{ __('offer_button_text }}') }}<i
