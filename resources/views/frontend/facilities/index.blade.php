...
@section('content')
    <main class="wexnix_main">
        ...
        <div class="wexnix_site-breadcrumb" style="background: url({{ $pageSettings->breadcrumb_image_url ?? '/frontend/assets/img/breadcrumb/01.jpg' }})">
            <div class="container">
                <h2 class="wexnix_breadcrumb-title">{{ $pageSettings->{{ __('breadcrumb_title ?: \'Our Facilities\' }}') }}</h2>
                <ul class="wexnix_breadcrumb-menu">
                    <li><a href="{{ route('home') }}">{{ __('Home') }}</a></li>
                    <li class="active">{{ $pageSettings->{{ __('breadcrumb_title ?: \'Our Facilities\' }}') }}</li>
                </ul>
            </div>
        </div>
        <!-- breadcrumb end -->
        ...
        <div class="wexnix_facility-area py-120">
            <div class="container">
                ...
                @if ($facilities->{{ __('count())') }}
                    ...
                    <!-- pagination -->
                    {{ $facilities->{{ __('links(\'frontend.pagination.wexnix\') }}') }}
                    <!-- pagination end -->
                @else
