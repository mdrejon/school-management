...
@section('content')
    <main class="wexnix_main">
        ...
        <div class="wexnix_site-breadcrumb" style="background: url({{ $pageSettings->breadcrumb_image_url ?? '/frontend/assets/img/breadcrumb/01.jpg' }})">
            <div class="container">
                <h2 class="wexnix_breadcrumb-title">{{ $pageSettings->breadcrumb_title ?: 'Our Blog' }}</h2>
                <ul class="wexnix_breadcrumb-menu">
                    <li><a href="{{ route('home') }}">{{ __('Home') }}</a></li>
                    <li class="active">{{ $pageSettings->breadcrumb_title ?: 'Our Blog' }}</li>
                </ul>
            </div>
        </div>
        <!-- breadcrumb end -->
        ...
        <div class="wexnix_blog-area py-120">
            <div class="container">
                ...
                @if ($posts->count())
                    ...
                    <!-- pagination -->
                    {{ $posts->links('frontend.pagination.wexnix') }}
                    <!-- pagination end -->
                @else
