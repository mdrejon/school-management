...
@section('content')
    <main class="wexnix_main">
        ...
        <div class="wexnix_site-breadcrumb" style="background: url('{{ $pageSettings->breadcrumb_image_url ?: asset('assets/img/breadcrumb/01.jpg') }}')">
            <div class="container">
                <h2 class="wexnix_breadcrumb-title">{{ $pageSettings->{{ __('breadcrumb_title ?: \'Class Schedule\' }}') }}</h2>
                <ul class="wexnix_breadcrumb-menu">
                    <li><a href="{{ route('home') }}">{{ __('Home') }}</a></li>
                    <li class="active">{{ $pageSettings->{{ __('breadcrumb_title ?: \'Class Schedule\' }}') }}</li>
                </ul>
            </div>
        </div>
        <!-- breadcrumb end -->
            ...
            <div class="container">
                ...
                <div class="wexnix_schedule-header-card text-center mb-5 p-4 rounded-4 shadow-sm bg-light-subtle">
                    <h2 class="wexnix_schedule-main-title font-weight-bold color-primary mb-2">
                        {{ $pageSettings->{{ __('breadcrumb_title ?: \'Class Schedule\' }}') }}
                    </h2>
                    ...
                    <div class="col-lg-10">
                        <div class="wexnix_schedule-content-box p-4 p-md-5 bg-white rounded-4 border shadow-sm">
                            <h4 class="wexnix_schedule-section-heading mb-4 font-weight-bold text-dark">
                                {{ $pageSettings->{{ __('section_title ?: \'Daily Activities\' }}') }}
                            </h4>

                            @if (!empty($pageSettings->{{ __('section_description))') }}
                                <p class="text-muted mb-4">{{ $pageSettings->{{ __('section_description }}') }}</p>
                            @endif
                            ...
                            <div class="wexnix_schedule-list-wrapper">
                                ...
                                @empty
                                    <div class="text-center py-4 text-muted">
                                        <p>{{ __('No class schedule available at this time.') }}</p>
                                    </div>
                                @endforelse
                            </div>
                        </div>
