...
class DepartmentPageSetting extends Model
{
    ...
    {
        static::saved(fn () => Cache::forget('department_page_settings.current'));
    }
    ...
    {
        $row = Cache::rememberForever('department_page_settings.current', function () {
            $settings = static::query()->first();

            if (! $settings) {
                $settings = static::create([
                    'section_tagline' => ['en' => 'Department'],
                    'section_title' => ['en' => 'Browse Our Department'],
                    'section_highlight' => ['en' => 'Department'],
                    'breadcrumb_title' => ['en' => 'Academics Department'],
                ]);
            }
